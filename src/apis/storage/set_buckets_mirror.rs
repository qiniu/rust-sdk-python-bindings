...
pub(super) fn create_module(py: Python<'_>) -> PyResult<&PyModule> {
    let m = PyModule::new(py, "set_buckets_mirror")?;
    m.add_class::<Client>()?;
    Ok(m)
}
#[doc = "设置存储空间的镜像源"]
...
impl Client {
    ...
    #[doc = "发出阻塞请求"]
    #[pyo3(
        text_signature = "(endpoints, credential, /, use_https, version, headers, query, query_pairs, appended_user_agent, bucket, src_site_url, host, uploading_progress, receive_response_status, receive_response_header, to_resolve_domain, domain_resolved, to_choose_ips, ips_chosen, before_request_signed, after_request_signed, response_ok, before_backoff, after_backoff)"
    )]
    #[args(
        r#use_https = "None",
        r#version = "None",
    ...
    fn call(
        ...
        r#bucket: Option<String>,
        r#src_site_url: Option<String>,
        ...
        let (resp, parts) = super_._call(
            ...
            version,
            {
                let mut segments = vec!["/image".to_owned()];
                segments.extend({
                    let mut segments = vec![];
                    if let Some(r#bucket) = r#bucket {
                        segments.push(r#bucket);
                    }
                    segments
                });
                segments.extend({
                    let mut segments = vec!["from".to_owned()];
                    if let Some(r#src_site_url) = r#src_site_url {
                        segments.push(qiniu_sdk::utils::base64::urlsafe(r#src_site_url.as_bytes()));
                    }
                    segments
                });
                segments.extend({
