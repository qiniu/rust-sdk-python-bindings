...
pub(super) fn create_module(py: Python<'_>) -> PyResult<&PyModule> {
    let m = PyModule::new(py, "fetch_object")?;
    m.add_class::<Client>()?;
    Ok(m)
}
...
impl Client {
    ...
    #[doc = "发出阻塞请求"]
    #[pyo3(
        text_signature = "(endpoints, credential, /, use_https, version, headers, query, query_pairs, appended_user_agent, from_url, to_entry, host, uploading_progress, receive_response_status, receive_response_header, to_resolve_domain, domain_resolved, to_choose_ips, ips_chosen, before_request_signed, after_request_signed, response_ok, before_backoff, after_backoff)"
    )]
    #[args(
        r#use_https = "None",
        r#version = "None",
    ...
    fn call(
        ...
        r#from_url: Option<String>,
        r#to_entry: Option<String>,
        r#host: Option<String>,
        ...
        let (resp, parts) = super_._call(
            "POST".to_owned(),
            endpoints,
            Some(vec![crate::http_client::ServiceName::r#Io]),
            use_https,
            version,
            {
                let mut segments = vec!["/fetch".to_owned()];
                segments.extend({
                    let mut segments = vec![];
                    if let Some(r#from_url) = r#from_url {
                        segments.push(qiniu_sdk::utils::base64::urlsafe(r#from_url.as_bytes()));
                    }
                    segments
                });
                segments.extend({
                    let mut segments = vec!["to".to_owned()];
                    if let Some(r#to_entry) = r#to_entry {
                        segments.push(qiniu_sdk::utils::base64::urlsafe(r#to_entry.as_bytes()));
                    }
                    segments
                });
                segments.extend({
                    let mut segments = vec!["host".to_owned()];
                    if let Some(r#host) = r#host {
                        segments.push(qiniu_sdk::utils::base64::urlsafe(r#host.as_bytes()));
                    }
                    segments
                });
                Some(segments.join("/"))
                ...
                ._async_call(
                    ...
                    version,
                    {
                        ...
                        segments.extend({
                            ...
                            if let Some(r#from_url) = r#from_url {
                                segments
                                    .push(qiniu_sdk::utils::base64::urlsafe(r#from_url.as_bytes()));
                            }
                            segments
                        });
                        segments.extend({
                            ...
                            if let Some(r#to_entry) = r#to_entry {
                                segments
                                    .push(qiniu_sdk::utils::base64::urlsafe(r#to_entry.as_bytes()));
                            }
                            segments
                        });
                        segments.extend({
