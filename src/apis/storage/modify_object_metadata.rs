...
pub(super) fn create_module(py: Python<'_>) -> PyResult<&PyModule> {
    let m = PyModule::new(py, "modify_object_metadata")?;
    m.add_class::<Client>()?;
    Ok(m)
}
#[doc = "修改文件元信息"]
...
impl Client {
    ...
    #[doc = "发出阻塞请求"]
    #[pyo3(
        text_signature = "(endpoints, credential, /, use_https, version, headers, query, query_pairs, appended_user_agent, entry, mime_type, condition, meta_data, uploading_progress, receive_response_status, receive_response_header, to_resolve_domain, domain_resolved, to_choose_ips, ips_chosen, before_request_signed, after_request_signed, response_ok, response_error, before_backoff, after_backoff)"
    )]
    #[args(
        r#use_https = "None",
        r#version = "None",
    ...
    fn call(
        ...
        r#condition: Option<String>,
        r#meta_data: Option<std::collections::HashMap<String, String>>,
        ...
        let (resp, parts) = super_._call(
            ...
            version,
            {
                let mut segments = vec!["/chgm".to_owned()];
                segments.extend({
                ...
                segments.extend({
                    let mut segments = vec!["mime".to_owned()];
                ...
                segments.extend({
                    let mut segments = vec!["cond".to_owned()];
                    if let Some(r#condition) = r#condition {
                        segments.push(qiniu_sdk::utils::base64::urlsafe(r#condition.as_bytes()));
                    }
                    segments
                });
                segments.extend({
                    let mut segments = Vec::new();
                    if let Some(free_params) = r#meta_data {
