...
pub(super) fn create_module(py: Python<'_>) -> PyResult<&PyModule> {
    let m = PyModule::new(py, "resumable_upload_v1_bput")?;
    m.add_class::<Client>()?;
    Ok(m)
}
...
impl Client {
    ...
    #[doc = "发出阻塞请求"]
    #[pyo3(
        text_signature = "(endpoints, upload_token, /, use_https, version, headers, query, query_pairs, appended_user_agent, ctx, chunk_offset, bytes, body, body_len, content_type, uploading_progress, receive_response_status, receive_response_header, to_resolve_domain, domain_resolved, to_choose_ips, ips_chosen, before_request_signed, after_request_signed, response_ok, before_backoff, after_backoff)"
    )]
    #[args(
        r#use_https = "None",
        r#version = "None",
    ...
    fn call(
        ...
        let (resp, parts) = super_._call(
            ...
            version,
            {
                let mut segments = vec!["/bput".to_owned()];
                segments.extend({
                    let mut segments = vec![];
                    if let Some(r#ctx) = r#ctx {
                        segments.push(r#ctx);
                    }
                    segments
                });
                segments.extend({
                    let mut segments = vec![];
                    if let Some(r#chunk_offset) = r#chunk_offset {
                        segments.push(r#chunk_offset.to_string());
                    }
                    segments
                });
                Some(segments.join("/"))
