...
pub(super) fn create_module(py: Python<'_>) -> PyResult<&PyModule> {
    let m = PyModule::new(py, "restore_archived_object")?;
    m.add_class::<Client>()?;
    Ok(m)
}
...
impl Client {
    ...
    #[doc = "发出阻塞请求"]
    #[pyo3(
        text_signature = "(endpoints, credential, /, use_https, version, headers, query, query_pairs, appended_user_agent, entry, freeze_after_days, uploading_progress, receive_response_status, receive_response_header, to_resolve_domain, domain_resolved, to_choose_ips, ips_chosen, before_request_signed, after_request_signed, response_ok, response_error, before_backoff, after_backoff)"
    )]
    #[args(
        r#use_https = "None",
        r#version = "None",
    ...
    fn call(
        ...
        r#freeze_after_days: Option<i64>,
        ...
        let (resp, parts) = super_._call(
            ...
            version,
            {
                let mut segments = vec!["/restoreAr".to_owned()];
                segments.extend({
                ...
                segments.extend({
                    let mut segments = vec!["freezeAfterDays".to_owned()];
                    if let Some(r#freeze_after_days) = r#freeze_after_days {
                        segments.push(r#freeze_after_days.to_string());
                    }
                    segments
                });
                Some(segments.join("/"))
