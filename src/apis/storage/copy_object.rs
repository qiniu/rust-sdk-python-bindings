...
pub(super) fn create_module(py: Python<'_>) -> PyResult<&PyModule> {
    let m = PyModule::new(py, "copy_object")?;
    m.add_class::<Client>()?;
    Ok(m)
}
...
impl Client {
    ...
    #[doc = "发出阻塞请求"]
    #[pyo3(
        text_signature = "(endpoints, credential, /, use_https, version, headers, query, query_pairs, appended_user_agent, src_entry, dest_entry, is_force, uploading_progress, receive_response_status, receive_response_header, to_resolve_domain, domain_resolved, to_choose_ips, ips_chosen, before_request_signed, after_request_signed, response_ok, response_error, before_backoff, after_backoff)"
    )]
    #[args(
        r#use_https = "None",
        r#version = "None",
    ...
    fn call(
        ...
        r#src_entry: Option<String>,
        r#dest_entry: Option<String>,
        r#is_force: Option<bool>,
        ...
        let (resp, parts) = super_._call(
            ...
            version,
            {
                let mut segments = vec!["/copy".to_owned()];
                segments.extend({
                    let mut segments = vec![];
                    if let Some(r#src_entry) = r#src_entry {
                        segments.push(qiniu_sdk::utils::base64::urlsafe(r#src_entry.as_bytes()));
                    }
                    segments
                });
                segments.extend({
                    let mut segments = vec![];
                    if let Some(r#dest_entry) = r#dest_entry {
                        segments.push(qiniu_sdk::utils::base64::urlsafe(r#dest_entry.as_bytes()));
                    }
                    segments
                });
                segments.extend({
                    let mut segments = vec!["force".to_owned()];
                    if let Some(r#is_force) = r#is_force {
                        segments.push(r#is_force.to_string());
                    }
                    segments
                });
                Some(segments.join("/"))
