...
pub(super) fn create_module(py: Python<'_>) -> PyResult<&PyModule> {
    let m = PyModule::new(py, "delete_bucket")?;
    m.add_class::<Client>()?;
    Ok(m)
}
#[doc = "删除指定的存储空间"]
...
impl Client {
    ...
    #[doc = "发出阻塞请求"]
    #[pyo3(
        text_signature = "(endpoints, credential, /, use_https, version, headers, query, query_pairs, appended_user_agent, bucket, uploading_progress, receive_response_status, receive_response_header, to_resolve_domain, domain_resolved, to_choose_ips, ips_chosen, before_request_signed, after_request_signed, response_ok, before_backoff, after_backoff)"
    )]
    #[args(
        r#use_https = "None",
        r#version = "None",
    ...
    fn call(
        ...
        let (resp, parts) = super_._call(
            ...
            version,
            {
                let mut segments = vec!["/drop".to_owned()];
                segments.extend({
