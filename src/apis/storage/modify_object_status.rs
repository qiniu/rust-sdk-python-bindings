...
pub(super) fn create_module(py: Python<'_>) -> PyResult<&PyModule> {
    let m = PyModule::new(py, "modify_object_status")?;
    m.add_class::<Client>()?;
    Ok(m)
}
...
impl Client {
    ...
    #[doc = "发出阻塞请求"]
    #[pyo3(
        text_signature = "(endpoints, credential, /, use_https, version, headers, query, query_pairs, appended_user_agent, entry, status, uploading_progress, receive_response_status, receive_response_header, to_resolve_domain, domain_resolved, to_choose_ips, ips_chosen, before_request_signed, after_request_signed, response_ok, response_error, before_backoff, after_backoff)"
    )]
    #[args(
        r#use_https = "None",
        r#version = "None",
    ...
    fn call(
        ...
        let (resp, parts) = super_._call(
            ...
            version,
            {
                let mut segments = vec!["/chstatus".to_owned()];
                segments.extend({
                ...
                segments.extend({
                    let mut segments = vec!["status".to_owned()];
                    if let Some(r#status) = r#status {
                        segments.push(r#status.to_string());
                    }
                    segments
                });
                Some(segments.join("/"))
